...
- v32 shop levels 1–3 + restocking by cooking, v33 market (decorations), v34 fix for shop CSS leaking into the cooking screens, v35 column arithmetic + levels 4–6, v36 parent skill stats + opening the shop from game-lilly (game-lilly v37 playroom tile). **Live: kitchen v37 (deployed 2026-09-24, local and live tests green), game-lilly v37.**
- v37 = fixes from the user's review: buttons wait for speech after buying / restocking, a picture hint on every first miss, auto-guided mode counts as help, level-1 market seller counts change back coin by coin, the stall waits for its pictures, the return link to Lilly's world is forgotten on a fresh visit. Unit tests (47) and `tests/shop.cjs` (including a real-sound section) passed; the full `tests/app.cjs` and the offline check also passed locally on v37; deployed 2026-09-24 and `tests/shop.cjs`, `tests/app.cjs` and the Lilly → shop round trip all passed against the live URLs.
...
## Restaurant and market (planned 2026-09-23; shop levels 1–3 + restocking in v32, market in v33, levels 4–6 + column arithmetic in v35)
...
v32 (2026-09-23) shipped Phases 1+2 together: "ร้านของหนู" card next to the free kitchen, levels 1–3 (count the food / take exact money from the customer's purse / give change by counting on), gentle retries with a guided mode, the seal coin bank, restocking by cooking (the recipe plays as usual, then the batch goes on the shelf and the photo into the book without counting as feeding), shop stats and a clear-shop button on the parent page. v33 (same day) added the market (Phase 3): tap the seal bank in the shop, the squirrel sells ten decorations (5–20 baht); level 1 pays freely and gets change back, level 2 pays exactly, level 3 alternates exact payment with paying a 10/20 and choosing how much change comes back. Bought decorations appear at fixed spots in the shop and can be put away or brought back by tapping them in the market. v34 fixed shop CSS leaking into the cooking screens (user report: oven jumped while held, ingredients looked odd). v35 (same day) added the column-arithmetic engine (`js/shop/column.js`, ported from game-lilly: no leading zeros, carry/borrow, independent and guided modes) and levels 4–6: add the price of two items, how many are left on the shelf, two-food orders paid with a 20-baht note (column subtraction help), and a "how many altogether" question after restocking. Shop class names must never collide with kitchen class names in either direction (`tests/shop/css.test.mjs`). v36 (same day) finished the plan: parent page shows shop level, sales, decorations and a per-skill table (times / first-try % / help used from the last 100 transactions), and game-lilly's playroom opens the shop directly (`?mode=restaurant&return=…`, return URL whitelisted to `/game-lilly/`, kept in sessionStorage; the kitchen home then shows "🌈 กลับโลกของลิลลี่" instead of the title). Open item for the user: try the launcher on the real iPad, especially if Lilly is installed to the home screen (iOS may give it separate storage).
