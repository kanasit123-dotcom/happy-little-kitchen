...
- Free kitchen (`RECIPES.free`, `free: true`): `freeadd` lets the child drop any of `FREE_INGREDIENTS` into the bowl (repeats allowed), `freecook` shows six machine buttons and holds like a cook step; the result is a "mystery dish" = `FREE_CONTAINER[kind]` as base + the ingredients as `stage.bits` arranged in a ring + a `tint-<kind>` filter class. Its decorate step uses all `ALL_TOPPINGS` in a two-row horizontal scroller. Reactions are random yum/love (sneeze/full rules still apply). Its home entry is the wide `.free-card` under the recipe grid; the icon `assets/dishes/free.png` is composed from existing art.
