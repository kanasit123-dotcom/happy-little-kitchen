...
- `css/shop.css` is loaded on every screen, so every selector must start with `.shop`, `.shop-layer` or a shop-only class. v33 shipped unscoped `.tray` / `.running` / `.pop` / `.nudge`, which bent the kitchen's ingredient tray and made the oven jump while held; v34 scoped everything and `tests/shop/css.test.mjs` now fails on any leaking selector.
- Levels 4–6 (v35): `core.makeOrder()` builds the order from the chosen checkpoint (`linesFor()`, falling back to other checkpoints and finally a one-item count when stock is short); `price` and `remaining` orders carry `math.problem` and go through `status: 'thinking'` (`enterThinking()` in `ui.js`) between picking and paying. `price` uses `column.solveColumn()` (independent first, guided on help or the second miss), `remaining` asks with pictures and three choices. Level 5 `change` orders pay with a 20 note; the drawer adds a 10-baht coin and guided mode offers `columnPopup()` (column subtraction in a popup). After restocking at level 4+ `shop.restockQuiz` asks how many there are altogether before the next customer.
- Column engine (`js/shop/column.js`): cell classes all start with `col-` because the kitchen stylesheet already uses `.carry`/`.slot`-style names; the shop's own classes were renamed the same way (`serve-tray`, `tray-slot`, `shop-bump`, `shop-running`, `mark-over`…). `tests/shop/css.test.mjs` fails if a shop class name matches any kitchen class name (except a short list of intentionally shared ones such as `action-btn`).
- Launch from game-lilly (v36): `?mode=restaurant` opens the shop at start-up; `?return=` is accepted only for `/game-lilly/` on the same origin (or `http://127.0.0.1:5173/` when both run locally), stored in sessionStorage (`happy-little-kitchen-return`), and the address is cleaned with `history.replaceState`. `api.exit` sends the shop's back button to Lilly's world when a return URL exists. The parent page reads the shop save directly (`readShop()`, `shopSkills()`), so keep the transaction fields (`checkpoint`, `attempts`, `usedHelp`, `type`) stable.
- Review fixes (v37): buttons that lead on (market "buy more / back to shop", the restock "sell it in the shop") appear only after the spoken lines finish; the first miss always shows a picture hint (`firstHint()` / `lightCoin()` in `ui.js`), the second opens guided mode and marks `usedHelp`; the level-1 market seller counts the change back coin by coin; the stall waits (max 2.5 s) for the decoration pictures; the return link to Lilly's world survives reloads and back/forward only, not a fresh visit. `tests/shop.cjs` has one section with real sound on to prove the buttons wait for speech. `design/voice.py` retries and skips phrases edge-tts refuses (it refused "ลูกค้าขอ", so the shop says "ลูกค้าสั่ง").
- Levels 7–8 (v39): `PRODUCTS[x].from` gates cake/smoothie to level 7+ (`productsFor()`, `sellable()`, `canRestock()`), `unit: 'glass'` makes the shop say "แก้ว" for smoothies, `noteFor()` picks 10/20/50/100, totals are capped at 99. Big change uses `countOnStops()` for the number line and `suggestChange()` for the glowing coin; `#col-sub` is hidden for 100 because column subtraction from 100 needs a double borrow. Decorations have `set: 1|2`; `decorSetsOpen()` opens set 2 when set 1 is owned, and the market shows picture tabs.
